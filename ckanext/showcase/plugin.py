...
class ShowcasePlugin(plugins.SingletonPlugin, lib_plugins.DefaultDatasetForm):
    ...
    def before_map(self, map):
        ...
        with SubMapper(map, controller='ckanext.showcase.controller:ShowcaseController') as m:
            m.connect('ckanext_showcase_index', '/project', action='search',
                      highlight_actions='index search')
            m.connect('ckanext_showcase_new', '/project/new', action='new')
            m.connect('ckanext_showcase_delete', '/project/delete/{id}',
                      action='delete')
            m.connect('ckanext_showcase_read', '/project/{id}', action='read',
                      ckan_icon='picture')
            m.connect('ckanext_showcase_edit', '/project/edit/{id}',
                      action='edit', ckan_icon='edit')
            m.connect('ckanext_showcase_manage_datasets',
                      '/project/manage_datasets/{id}',
                      action="manage_datasets", ckan_icon="sitemap")
            m.connect('dataset_showcase_list', '/dataset/projects/{id}',
                      action='dataset_showcase_list', ckan_icon='picture')
            m.connect('ckanext_showcase_admins', '/ckan-admin/project_admins',
                      action='manage_showcase_admins', ckan_icon='picture'),
            m.connect('ckanext_showcase_admin_remove',
                      '/ckan-admin/project_admin_remove',
